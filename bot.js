...
class AdminBot {
    ...
    async handleChannelPost(msg) {
        try {
            ...
            // Маршрутизация только по каналу-приёмнику (msg.chat.id), не по forward_from_chat
            if (!channel.vk_group_id) {
                logger.warn(
                    `channel_post: chatId=${chatId} не привязан к ВК группе — кросс-постинг пропущен` +
                    (msg.forward_from_chat ? ` (переслано из ${msg.forward_from_chat.id})` : "")
                )
                return
            }

            const vkBridgeForChannel = this.vkBridges.find(b =>
                String(channel.vk_group_id) === String(b.vkGroupId)
            )

            if (!vkBridgeForChannel) {
                logger.warn(
                    `channel_post: chatId=${chatId} привязан к vk_group_id=${channel.vk_group_id}, ` +
                    `но VK Bridge для этой группы не найден — кросс-постинг пропущен`
                )
                return
            }

            logger.info(
                `channel_post: chatId=${chatId} → VK group ${vkBridgeForChannel.vkGroupId}` +
                (msg.forward_from_chat ? ` (forward from ${msg.forward_from_chat.id})` : "")
            )
